...
学習（`training.train`）と推論/配信（`serving.ai_server`）が画面に出すログの読み方をまとめる。
...
```
2026-06-12 21:11:39,186 [training.train] Beginning training...
└─── 日付と時刻 ───┘     └─ 出どころ ─┘ └──── 内容 ────┘
```
...
| カッコ内の名前 | 出どころ |
|-----------|---------|
| `training.train` | 学習プログラム |
| `serving.ai` | 配信用の AI サーバ |
...
## 1. 学習ログ（`training.train`）

### 1-1. 起動とステージ進行で出る行

既定では**オートカリキュラム（Stage 1→4、`--target-stage 4` 既定）**。進行は**固定ステップ制**で、
各ステージは決められた予算だけ走って必ず次へ進む（**卒業判定は無い**）。ログは「全体の起動まとめ」→
「ステージごとの開始・終了」の繰り返し。**いま何ステージ目か**は `=== Stage N: ... ===` の行で分かる。
...
2026-... [training.train] Stage 1 終了 (60000 steps 消化, 通算 60000)。指標: success_rate=0.42, all_placed=7回 (直近30: 0.13), episodes=241   ← 予算を使い切って次へ
2026-... [training.train] === Stage 2: Stage 2: cube だけ・高い目標 ===
2026-... [training.train] Inventory: {'cube': 15}, target_height=0.450 (=満積み×0.60), ...
   …（Stage 5 まで繰り返し）…
```

| 行 | 意味・見るところ |
|----|------|
| `SAC (curriculum)` | カリキュラム ON（既定）。`--no-curriculum` 実行なら `(single-stage: Stage 1...)` |
| `Curriculum: 固定ステップ制 ... ステージ予算 {...}` | 各ステージに割り当てたステップ数と合計。`--stage-steps` で上書きできる |
| **`=== Stage N: ... ===`** | **いま学習中のステージ番号**。ここでステージが切り替わる |
| `Inventory: ..., target_height=0.240 (=満積み×0.60)` | そのステージの在庫と目標高さ（= 在庫満積み高さ × ratio） |
| `Beginning training (stage N; このステージ X steps / 通算 A → B / 全体 Y)` | そのステージの予算 X と、走り終えた時点の通算ステップ B |
| `Stage N 終了 (X steps 消化, 通算 B)。指標: ...` | 予算を使い切って次ステージへ。**成績によらず必ず進む**（卒業判定は無い） |
| `指標: success_rate=... , all_placed=N回 (直近30: r)` | success_rate=「目標高さ到達」率（高さ指標）、all_placed=「全ブロックが1つの連結構造」回数（**高さ非依存**）。次回のステップ配分を決める材料 |
...
少し進むごとにこの表が出る。**平均報酬（`rollout/ep_rew_mean`）が伸びているか**が一番の見どころ。
グラフで見たいときは TensorBoard（§4）へ。
...
|    success_rate    | 0.40     |   ← 直近30エピソードの「目標高さ到達」率
|    all_placed_rate | 0.13     |   ← 直近30の「全部が1つの構造になった」率（高さ非依存）
|    all_placed_total| 7        |   ← このステージでの通算回数
|    episodes_seen   | 24       |
| rollout/           |          |
|    ep_rew_mean     | -1.09    |   ← 1ゲームあたりの平均報酬。**伸びていれば学習が進んでいる**
|    ep_len_mean     | 20.7     |   ← 1ゲームの平均手数
|    success_rate    | 0.45     |   ← SB3 自動。直近~100エピソードの成功率
| time/              |          |
|    episodes        | 24       |
|    fps             | 2        |
|    time_elapsed    | 192      |
...
#### `curriculum/`（いまどのステージか・上達の度合い）— カリキュラム実行時のみ

**これらは進行を左右しない記録専用の指標**（ステージは固定ステップ制で進む）。
次にどのステージへ何ステップ割り当てるかを決める材料として見る。

| 項目 | 意味 | 見方 |
|-----------|------|------|
| `stage` | いま学習中のステージ番号（1〜5） | スクロール中でも一目で分かる |
| `success_rate` | 直近 30 エピソードの「目標高さ到達」率 | **高さの指標**。伸びていれば高く積めるようになっている |
| `all_placed_rate` | 直近 30 の「全ブロックが1つの連結構造になった」率 | **高さ非依存**。「作品が1つにまとまった」度合い |
| `all_placed_total` | このステージでの `all_placed` 通算回数 | 次回のステップ配分を決める主要な材料 |
| `episodes_seen` | このステージで終わったエピソード数 | 30 に満たないうちは各 rate が安定しない |

> **`all_placed` は「高く積めた」ではない**。横に広く低い構造でも成立する
> （レンガ積み8個・高さ0.100m でも真になることを実測確認）。高さを見たいときは
> `success_rate` と併読すること。かつてこれを卒業条件に使っていたため
> ステージが不当に飛ぶ不具合があった（[`design_change_record.md`](design_change_record.md) §1.2.1）。

> **`rollout/success_rate` も出るが別物**: 窓が直近 **約100** エピソードなので
> `curriculum/success_rate`（30）とは数値が少しズレる。長い目で見た参考値。
...
```
（全ステージ走破後、output\training\fresh\sac_<YYYYMMDD-HHMMSS>_<手数>_steps.zip が 1 本保存される）
2026-... [training.train] 学習完了: 最終モデル = fresh/ の最大ステップ checkpoint (sac_final.zip は廃止)
2026-... [training.train] 長期記憶を保存: output\training\replay_buffer.pkl
2026-... [training.train] Resume state saved: output\training\resume_state.json (next_stage=2, completed=[1])
```
...
- **`replay_buffer.pkl`（長期記憶）と `resume_state.json`** は毎回保存される。
  live_server がスナップショット引き継ぎに使う（train 側の `--resume` は廃止済み）。
...
## 2. 配信 / AI サーバのログ（`serving.ai`）

### 2-1. 起動

```
2026-... [serving.ai] demo stage: id=5 '...' inventory={...} target=0.444 h_high=0.320 h_low=0.120  ← 常に最終ステージ
2026-... [serving.ai] loading model: output\training\fresh\sac_20260627-143022_4000_steps.zip   ← 無指定なら find_latest_checkpoint で最新 run の最大ステップを自動選択
2026-... [serving.ai] model loaded: n_params=856178      ← AI の頭脳の部品数（壊れてなければ毎回同じ数）
2026-... [serving.ai] short-term memory length: 5         ← 学習時と同じ「直近 5 手の記憶」設定
2026-... [serving.ai] setting up world                    ← 積み木の世界を準備
2026-... [serving.ai] spawned 15 blocks for stage 5: {...}  ← そのステージの在庫どおりに全形状を散布
...
```
2026-... [serving.ai] AI: pickup body=7 (0.51,-0.35,0.02) → place (-0.75,0.62,0.07), tower=0.050, lift=0.075
2026-... [serving.ai] post-action: tower_h=0.050, best=0.050, event=height_record
```
...
```
2026-... [serving.ai] no scattered block to pick — waiting 2.0s
        → 床に拾えるブロックが無い／遠くて、いったん待っている。

2026-... [serving.ai] COLLAPSE (h_before=0.105, h_after=0.020, dispersion=0.80)
...
| **カリキュラム** | — | Stage 1→4（既定）と難易度を段階的に上げながら学習するしくみ。進行は**固定ステップ制**（`--target-stage` は走る範囲の上限） |
| **ステージ / stage** | — | 難易度の段（1〜5）。段が上がると在庫数・目標高さ・形状が増える |
| **ステージ予算 / stage steps** | — | 各ステージに割り当てた学習ステップ数。**これを使い切ったら次へ進む**（成績は無関係）。`--stage-steps` で変更 |
| **success_rate（成功率）** | — | 直近 30 エピソードのうち「目標高さ到達」の割合。**進行には影響しない記録専用の指標** |
| **all_placed（散布0）** | — | 全ブロックが1つの連結構造になったか。**高さは問わない**ので低く広い構造でも成立する。記録専用 |
| **目標高さ / target_height** | — | そのステージのゴール高さ。＝ 在庫を全部縦積みした高さ × `ratio`（既定 0.6） |
| **成功（is_success）** | — | そのエピソードで「目標高さ到達」したか（success_rate の対象） |
...
- 学習プログラム: [`src/block_stacker/training/train.py`](../src/block_stacker/training/train.py)
- AI サーバ: [`src/block_stacker/serving/ai_server.py`](../src/block_stacker/serving/ai_server.py)
