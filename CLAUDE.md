...
- 学習（カリキュラム既定 ON, Stage1→5）:
  ...
  - 保存は **`output/mvp2/fresh/sac_<YYYYMMDD-HHMMSS>_<手数>_steps.zip`**（`sac_final.zip` は廃止）。
    日時プレフィックスは run 開始時刻で、同一 run の 5 本が共有する。`played/` 蓄積時のファイル名衝突を防ぐ。
    例: `sac_20260627-143022_3990_steps.zip`。ソート基準は **(run_ts, steps) 昇順**（古い run → 新しい run、
    同 run 内はステップ昇順）。`find_latest_checkpoint` は最新 run の最大ステップを返す。
  - checkpoint は **`total_timesteps` を `checkpoint_splits`（既定 5）等分した地点**で保存される
    （`configs/training.yaml` の `sac.checkpoint_splits` で変更可）。
    `save_freq = total_timesteps // splits // n_envs` で算出。4k・n_envs=6 なら
    ≈798 / 1596 / 2394 / 3192 / 3990 steps（約 20/40/60/80/100%）の 5 本が `fresh/` に生成される。
