...
- `--target-stage`（既定 4）/ `--max-stage` は**走る範囲の上限**。「到達で終了」ではない
- `all_placed`（散布0）は**記録専用の指標**。高さ非依存なので `all_placed_height` と必ず併読
- 既定予算 60k / 35k / **10k** / 60k / 70k（Stage1-4 = 165,000、全5 = 235,000）
...
- **プリセット標準レシピ**: `--start-stage 3 --target-stage 3`（Stage3 既定 10,000 steps）。
  「不器用→習得」の壁（実測 12k〜15k）の手前で意図的に止める。
  ⚠️ **ステージ範囲は既定ではない**ので、この2フラグの明示が必要
...
- **AWS スケジュールは暫定**: プリセット生成=月初1日 10k、学習配信=平日10-18。確定値ではない
...
`stages[].steps` の**現行**既定値。合計 235,000（既定範囲 Stage 1-4 は 165,000）。

| Stage | steps | 根拠 |
|---|---|---|
| 1 | 60,000 | ゼロから基礎獲得。以降の全ステージがこの方策を継承する |
| 2 | 35,000 | 同じ cube のまま個数と高さが増えるだけ。転移が最も効く |
| 3 | **10,000** | **プリセット生成（Stage3 のみ）の標準値を兼ねる**。習得の壁の手前で止める（§1.7） |
| 4 | 60,000 | 三角柱＝斜面。実測で退行が出るため厚め |
| 5 | 70,000 | 円柱＝転がる。既存方策が通用しない新規スキル |

Stage 1（ゼロから）と Stage 4/5（斜面・曲面）が重い。**Stage 3=10k が極端に小さいのは、この値が
プリセット生成の標準ステップ数を兼ねているため**（全 run で回すには短すぎる。要 --stage-steps 上書き）。

> 変遷: 当初 Stage3=40k → 実測で 25k に削減 → 最終的にプリセット標準を兼ねて 10k。
> 総量は当初 250k（2 steps/秒 で ~35h）想定だったが、n_envs=1 実測 約2.5 steps/秒 で 235k ≈ 26h。
...
> Stage 3 を 40,000→25,000→（最終）10,000 に削り、Stage 4 を 45,000→60,000 に増やした。
...
### 1.7 プリセットの標準を「壁の手前」に（Stage 3 のみ・10,000 steps）

デフォルトの全ステージ学習とは別に、**ライブ配信用プリセットの標準レシピ**を
`--start-stage 3 --target-stage 3`（Stage 3 の既定 steps=10,000）に定めた。当初 12,000 としたが、
Stage 3 の config 既定値そのものを 10,000 にして `--stage-steps` 不要にした（月初の自動生成で扱いやすい）。
...
**10,000 で壁の手前に止める**と「掴む・運ぶはできるが積めない子供」になり、ライブ（Stage 5）で
