...
- **用途**: SAC（Soft Actor-Critic）アルゴリズムの実装基盤。`training/train.py` が `SAC` クラスを使い、`MultiInputPolicy` と `HybridFeatureExtractor` を組み合わせて学習を実行。`CheckpointCallback`（定期保存）・`GraduationCallback`（カリキュラム卒業検出）も SB3 の Callback API を継承して実装。
...
難易度を段階的に上げていく学習スケジューリングの仕組み（`training/curriculum.py`）。最初は簡単なステージで基礎を学ばせ、卒業条件を満たしたら次の難易度へ進む。

- **用途**: Stage 1（直方体のみ）→ Stage 2（三角柱追加）→ Stage 3（多様な形状）→ Stage 4（難度高）→ Stage 5（全形状・円柱含む）の 5 段階。`GraduationCallback` が直近 30 エピソードの成功率（≥ 0.6）または散布ブロックゼロを検出して卒業を判定。
...
- **用途**: `tests/` 以下。`tests/test_train.py`（13 テスト、`_compute_save_freq` や CLI パーサーの単体テスト）・`tests/test_live_server.py`（84 テスト、`live_server.py` の各コンポーネント）を管理。
