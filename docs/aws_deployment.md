...
現状は Redis 不使用（S3 で世界状態と model checkpoint を永続化、WebSocket セッションは live_server in-process）。
...
3. **学習→推論のモデル更新を `live_server` の `--sync-every` より細かく制御したい**
   `live_server` は `WeightSyncer` で in-process 同期（既定 `--sync-every`）。外部プロセス間での即時反映（< 1 分）が必要なら Redis pub/sub を検討。
...
| `src/block_stacker/serving/stm.py` | 推論時の `ShortTermMemory` クラス |
