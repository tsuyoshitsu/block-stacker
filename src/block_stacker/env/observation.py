...
- **flat** (MVP 1 compat): a single (max_blocks*per_block_dims + 1,) float32
  vector. 散布ブロックを距離順で並べ、`max_blocks` を超えた分は drop、空き枠は 0 padding。

- **dict** (MVP 2): a dict suitable for a Set Transformer + CNN policy:
...
設計上のポイント:
    - 並び: タワー base からの 2D 距離でソート → top-K に切り詰め。
      MVP 2 の Set Transformer は順序不変なので並びは無関係だが、
      MVP 1 の flat 用には near-first で意味のある並びが必要。
...
def observation_dim(max_blocks: int, n_shapes: int) -> int:
    """Flat observation dim. Kept for MVP 1 compatibility."""
    return per_block_dims(n_shapes) * max_blocks + 1
...
def pack_observation_flat(
    ...
    """Flat (MVP 1) observation: one big float32 vector."""
    pb_dims = per_block_dims(n_shapes)
    obs = np.zeros((observation_dim(max_blocks, n_shapes),), dtype=np.float32)
    for i, (dist, block, pos, quat, in_tower) in enumerate(
        _sorted_block_rows(blocks, tower_block_ids, reference_xy)[:max_blocks]
    ):
        base = i * pb_dims
        _pack_one(block, pos, quat, dist, in_tower, shape_index, n_shapes,
                  obs[base:base + pb_dims])
    obs[-1] = float(tower_top_z)
...
def pack_observation_dict(
    ...
    """Dict (MVP 2) observation: structured for Set Transformer + CNN."""
    pb_dims = per_block_dims(n_shapes)
