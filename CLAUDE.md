...
- 学習（カリキュラム既定 ON, Stage1→5）:
  ...
  - 保存は **`output/mvp2/fresh/sac_<手数>_steps.zip`**（`sac_final.zip` は廃止）。
    ステージ跨ぎで連続した通算ステップ数。`tools/demo_checkpoints.ps1` / `local_loop.ps1` がこれを参照。
...
- 前回の学習を引き継いで再開（`--resume`）:
  ...
  - **勘（NN重み）**: `fresh/` か `played/` の最大ステップ checkpoint を自動選択して `SAC.load()`（無加工）。
...
- ローカル成長観察（開発用）: `tools/local_loop.ps1`（`fresh/` を昇順に1巡再生して終了。`played/` 移動なし）。
