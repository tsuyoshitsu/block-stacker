...
┌──────────────────────────────────────────────────────────┐
│ 1. 学習を回す                                            │
│    python -m block_stacker.training.train    ← 引数なし=プリセット │
...
**既定は「プリセット生成」（Stage 3 のみ・5,000 steps）**。`--start-stage` / `--target-stage` の
既定がどちらも 3 なので、**引数なしで実行するとプリセットが 1 本できる**。
進行は**固定ステップ制**で、複数ステージを走らせた場合は決められたステップ数だけ走り、
**成績によらず**次のステージへ進む（卒業判定は廃止）。

> ⚠️ **フルカリキュラムは引数なしでは走らない**。`--start-stage 1 --target-stage 4` を明示すること。
...
| 3 | +cuboid | **5,000** | **プリセット生成の標準値**（train 既定がここを走る）。全 run では短すぎるので上書き前提 |
| 4 | +三角柱 | **60,000** | **斜面**。実測で 0.43 → 0.00 に退行し、回復途中で予算切れになった |
| 5 | +円柱 | **70,000** | **曲面**（転がる）。既存方策が通用しない新規スキル |
| | **合計** | **230,000** | Stage 1-4 は **160,000**（明示指定時） |
...
> `--n-envs 1 --sync-every 50` は**既定値と同じ**なので省略可（上は明示した形）。
> `--n-envs` を変える場合はスナップショットの n_envs と一致させること
> （不一致だと学習スレッドだけが落ちて配信は生き残る。詳細は [`live_mode.md`](live_mode.md)）。
...
| **ライブ用プリセット（既定・Stage 3 のみ 5k）** | （フラグなし） | **約 35 分** | プリセット 1 本 |
| フルカリキュラム（Stage 1→4, 計16万） | `--start-stage 1 --target-stage 4` | 約 18 時間 | プリセット 1 本 |
| Stage 5 完走（全形状, 計23万） | `--start-stage 1 --target-stage 5` | 約 26 時間 | プリセット 1 本 |
