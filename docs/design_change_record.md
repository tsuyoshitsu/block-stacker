...
- **推奨実行値 `--n-envs 1 --sync-every 50`**（コード既定 4 / 500 とズレている。§0 末尾の注意参照）
...
- **live_server のコード既定値が運用値とズレている**: 既定 `--n-envs 4` / `--sync-every 500` に対し
  運用は 1 / 50。**n_envs 不一致だと `AssertionError` で学習スレッドだけが落ち、配信は生き残る**
  （＝賢くならないことに気づきにくい）。コードは未変更で、docs 側で吸収している
- **報酬設計は未着手**: `place_success` の高さ補正が残っており、「高さ非依存で建造物を楽しむ」
  方針とは未整合
