...
既定では**オートカリキュラム（Stage 1→4 を自動進行、`--target-stage 4` 既定）**。ログは「全体の起動まとめ」→
「ステージごとの開始・卒業」を Stage 4 まで繰り返す（`--target-stage 5` で Stage 5 まで）。**いま何ステージ目か**は
`=== Stage N: ... ===` の行で分かる。
...
2026-... [training.train] Stage 1 graduated (success_rate=0.78).            ← 卒業！次ステージへ
...
```
（学習中、output\training\fresh\sac_<YYYYMMDD-HHMMSS>_<手数>_steps.zip が total_timesteps の 20/40/60/80/100% 地点で自動保存）
...
- checkpoint は `checkpoint_every`（既定 50000）ステップ間隔で生成される。
  `--target-stage`（既定 4）卒業時には追加で明示的 checkpoint も保存される。
...
| **カリキュラム** | — | Stage 1→4（既定）と難易度を段階的に上げながら学習するしくみ（`--target-stage 4` 既定 ON。`--target-stage 5` で Stage 5 まで進む） |
