...
- 学習（カリキュラム既定 ON, Stage1→5）:
  ...
  - checkpoint は **`total_timesteps` を `checkpoint_splits`（既定 5）等分した地点**で保存される
    ...
    ≈798 / 1596 / 2394 / 3192 / 3990 steps（約 20/40/60/80/100%）の 5 本が生成され、
...
- 週次配信モード（月〜金 5 日サイクル）:
  1. 日曜: `tools/curate_week.ps1`（checkpoint を生成分そのまま採用 → `output/weeks/<YYYY-WNN>/`）
  2. 平日 14:00: `tools/advance_day.ps1`（当日の step モデルで ai_server を起動・切り替え）
  3. 土・日: 最後の step_05.zip で固定表示。次の日曜に新サイクル。
  - 学習側が `total_timesteps` を `checkpoint_splits(=5)` 等分した地点で 5 本生成するのが正。
    curate_week は再分割せずそのまま採用（5 本超の場合は最新 5 本、不足は sac_final.zip でパディング）。
  - `curate_week.ps1 -MaxSteps <N>`: N ステップ以下の checkpoint だけを採用対象にする。
