...
- **flat** (旧形式、後方互換): a single (max_blocks*per_block_dims + 1,) float32
  vector. 散布ブロックを距離順で並べ、`max_blocks` を超えた分は drop、空き枠は 0 padding。

- **dict** (現在の形式、Set Transformer + CNN 用):
...
設計上のポイント:
    - 並び: タワー base からの 2D 距離でソート → top-K に切り詰め。
      dict 形式の Set Transformer は順序不変なので並びは無関係だが、
      flat 形式では near-first で意味のある並びが必要。
...
def observation_dim(max_blocks: int, n_shapes: int) -> int:
    """Flat observation dim. Kept for backward compatibility."""
...
def pack_observation_flat(
    ...
    """Flat (旧形式) observation: one big float32 vector."""
